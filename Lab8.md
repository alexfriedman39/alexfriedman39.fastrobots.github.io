...
The graphs shown below display both the robot's distance from the wall and its motor's PWM value over time. The plots correspond to flips 1 and 2, respectively. In both flips 1 and 2, the distance data is approximately parabolic around zero. This occurs because the robot flips over its front wheels, causing the distance sensor to pass over the ground before switching directions. It is evident that the flip occurs during this time period due to the abrupt change in PWM values. 

In addition, the distance plot for flip 2 shows a drastic increase in distance readings after the flip, which occurs because the TOF sensor essentially "switches" sides on the car. As a result, it now senses that it is a far distance from the nearest wall. This jump in distance measurements is not seen in the flip 1 data, likely because the TOF sensor could not accurately find the nearet wall since the door was open during this run. 
