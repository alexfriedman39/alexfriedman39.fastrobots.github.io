...
The graphs shown below display both the robot's distance from the wall and its motor's PWM value over time. The plots correspond to flips 1 and 2, respectively. In both flips 1 and 2, the distance data is approximately parabolic around zero. This period of time is when the flip occurs, which is evident by the abrupt change in PWM values. This approximately parabolic behavior occurs because the robot flips over its front wheels, causing the distance sensor to pass over the ground before switching directions. The distance plot for flip 2 shows the drastic increase in distance readings after the flip, which occurs because the sensor essentially "switches" sides on the car, and now reads that it is a far distance from the nearest wall. This jump in distance measurements is not seen in the flip 1 data, likely because the TOF sensor could not accurately find the nearet wall since the door was open during this run. 
...
<p align="center">
<img width="500" src="photos/Lab8/Flip_2_Data.png">
</p>
<br>
